...
class ASRacos():
    def __init__(self, lower_dim, upper_dim):
        self.asracoscont = ASRacosCont(lower_dim, upper_dim,)
    ...
    def opt(self, objective, parameter, strategy='WR', ub=1):
        ...
        updater = Updater(objective, parameter, unevaluated_queue, evaluated_queue, result_queue, history_queue, self.asracoscont, self.lower_dim, self.upper_dim,
                          strategy, ub)
        updater.start()
...
class ASRacosCont(SRacos):
    ...
    def __init__(self, lower_dim, upper_dim):
        ...
        Initialization.
        """
        SRacos.__init__(self, lower_dim, upper_dim)
        return
...
class Updater(Process):
    def __init__(self, objective, parameter, unevaluated_queue, evaluated_queue, result_queue, history_queue, asracoscont, lower_dim, upper_dim, strategy='WR',  ub=1):
    ...
    def run(self):
        ...
        classifier = RacosClassification(
            self.objective.get_dim(), self.asracoscont.get_positive_data(), self.asracoscont.get_negative_data(), lower_dim=self.lower_dim, upper_dim=self.upper_dim, ub=self.ub)
