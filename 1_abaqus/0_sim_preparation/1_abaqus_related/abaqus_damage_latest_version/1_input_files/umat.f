...
c
     ...
     5          DHDGDG(ND,ND)
C     ------------------------------------------------------------------
C     add scalar damage variable
C     add material constants for damage
      real*8 gammaDamageMin, gammaDamageMax, gammaMax,
     2     damageVariable, damageMax, damageM
C     ------------------------------------------------------------------
...
c$$$      if(kinc .eq. 30) then
c$$$        write(*,*) "damageVariable=", STATEV(126-4)
c$$$      end if
...
C     ------------------------------------------------------------------
C     add scalar damage variable
C     read damageVariable from sdv
      damageVariable = STATEV(127-4)
C     scale dtausp by damageVariable
      DTAUSP = DTAUSP / (1.0 - damageVariable)
C     ------------------------------------------------------------------
...
C     ------------------------------------------------------------------
C     add scalar damage variable
C     GAMMA(NSLPTL) = STATEV(NSLPTL+1)
      call findGammaMax(STATEV(NSLPTL+1),NSLPTL, gammaMax)
C     
      gammaDamageMin = 0.1
      gammaDamageMax = 0.7
      damageM = 2.0
...
C     calculate Damage variable
      if(gammaMax .le. gammaDamageMin) then
        damageVariable = 0.0
      else if((gammaMax .gt. gammaDamageMin) .and.
     &       (gammaMax  .lt. gammaDamageMax)) then
        damageVariable = damageMax * (gammaMax-gammaDamageMin)**damageM
     &       /(gammaDamageMax - gammaDamageMin)**damageM
      else
        damageVariable = damageMax
      end if

      STATEV(127-4) = damageVariable
C     write(*,*) "damageVariable = ", damageVariable
C     write(*,*) "gammaMax=", gammaMax
C     ------------------------------------------------------------------

C     this is intended for deleting element
      if(damageVariable .ge. 0.5) then
	if(STATEV(127 - 5) .ne. 0 ) then
          STATEV(127 - 5) = 0
	end if
      end if
...
C----------------------------------------------------------------------
      subroutine findGammaMax(GAMMA,NSLPTL, gammaMax)
C     find maximum gamma in all slip systems
C     GAMMA  -- shear strain in all slip systems at the start of time
C               step  (INPUT)
C     gammaMax--maximum gamma in all slip systems (OUTPUT)
      implicit real*8 (A-H, O-Z)
      dimension GAMMA(NSLPTL)
      real*8 gammaMax

      gammaMax = 0.0
C     loop to find the maximum value in all slip systems
      do i = 1, NSLPTL
        if(abs(GAMMA(i)) .GT. gammaMax) then
          gammaMax = abs( GAMMA(i))
        end if
      end do
      end subroutine findGammaMax
C----------------------------------------------------------------------
