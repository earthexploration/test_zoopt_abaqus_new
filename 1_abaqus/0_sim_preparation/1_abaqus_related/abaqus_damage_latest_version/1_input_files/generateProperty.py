...
# sys.argv give parameters in unit Pa, therefore, it should be transformed
# into MPa (in Huang umat document and the above parameters, it seems these
# data are measured by Mpa)
c11  = float(sys.argv[1]) / 1.e6
c12  = float(sys.argv[2]) / 1.e6
c44  = float(sys.argv[3]) / 1.e6
#
n    = float(sys.argv[4])
adot = float(sys.argv[5])
#
# initial hardening modulus
h0   = float(sys.argv[6])
# saturation stress
taus = float(sys.argv[7])
# initial critical resolved shear stress
tau0 = float(sys.argv[8])
# ratio of latent to self-hardening: the same sets
q   = float(sys.argv[9])
# ration of laten to self-hardening: different sets
q1  = float(sys.argv[10])
# #
# n    = 10.0
# adot = 0.001
# #
